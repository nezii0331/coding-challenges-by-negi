...
class day36_class {
    ...
    public static Node removeDuplicates(Node head) {
        ...
        if (head != null) {
            // first i create a dummy to store head node
            Node dummyHead;
            Node toStore;

            dummyHead = head;
            while (dummyHead != null && dummyHead.next != null) {
                if (dummyHead.data == dummyHead.next.data) { // if right now head.value == next remove next headvalue
                    dummyHead.next = dummyHead.next.next;
                    // System.out.println("check head: " + dummyHead.data + " " +"check next: " +
                    // dummyHead.next.data);
                } else {
                    dummyHead = dummyHead.next; // move to next Node
                }
            }
        }
        return head;
